    ...
    while (WaitForDebugEvent(&de, INFINITE)) {
        if (de.dwDebugEventCode == CREATE_PROCESS_DEBUG_EVENT) {
            ...
            if (!system_bp_done) {
                ...
                printf("[*] Debugger ready. Loaded %d breakpoints.\n", g_bp_count);
                printf("[*] Commands: 'n' - next breakpoint, 'q' - quit\n");
            } else {
                ...
                int wait_cmd = 1;
                while (wait_cmd) {
                    printf("(dbg) Enter command: ");
                    if (scanf(" %c", &cmd) != 1) continue;

                    if (cmd == 'n') {
                        wait_cmd = 0; // Продолжаем выполнение до следующей точки
                    }
                    else if (cmd == 'q') {
                        ...
                        printf("[!] Unknown command '%c'. Use 'n' (next) or 'q' (quit).\n", cmd);
                    }
                }
